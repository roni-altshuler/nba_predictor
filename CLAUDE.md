...
Design language is **Bugatti, Arena variant** (owner-directed re-theme 2026-08-25): warm hardwood dark instead of neutral black — canvas `#0c0705`, surfaces `#17100a`/`#211711`, warm hairlines `#362718`, warm-white uppercase letterspaced display, monospace for nav/buttons/tables, and `--accent-brand` basketball orange `#e2682a` on CHROME ONLY (wordmark, active nav, tab underline, focus ring — never on data). **No gradients, no shadows, no glassmorphism on components.** Colour on content carries meaning only. The decorative exception is the **ambient hardwood layer** (`AmbientBackground.tsx` ember glows + `CourtField.tsx`, a perpetual dim chalk five-on-five — drives, steals, fast breaks — with NO digits ever): the whole court stays in frame, centred, on every device (portrait rotates it upright) and the camera only breathes (zoom ≤1.03×) — the frame never chases the play; players ≤0.29 / ball ≤0.39 alpha, `soft` by default with a reader's `data-ambient` dial (soft · vivid · off), ~30fps, negative z-index, framed still under reduced motion — see docs/DESIGN.md §6/§6a for guardrails before touching it.
